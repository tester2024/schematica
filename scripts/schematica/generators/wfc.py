...
or raises ``ContradictionError`` within an iteration cap. Build a fresh
``TileSet`` for each project so the output is generated from the requested
palette and constraints instead of a bundled preset.
...
def tileset_wildcard(tiles: list[str]) -> TileSet:
    ...
    other (all-``"*`` edges). Useful when the agent has generated a project-
    specific block palette and wants WFC-style seeded selection without a fixed
    preset.
    """
